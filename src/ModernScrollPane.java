...
import java.awt.*; /** * This is an implementation of a JScrollPane with a modern UI * * @author Philipp Danner * */ 
...
public ModernScrollPane(Component view, int vsbPolicy, int hsbPolicy) { 
    setBorder(null); 
    // Set ScrollBar UI JScrollBar verticalScrollBar = getVerticalScrollBar(); 
    ...
    setLayout(new ScrollPaneLayout() { private static final long serialVersionUID = 5740408979909014146L; 
    
    @Override 
    public void layoutContainer(Container parent) { 
    Rectangle availR = ((JScrollPane) parent).getBounds(); 
    Insets insets = parent.getInsets(); 
    availR.x = availR.y = 0; // viewport Insets insets = parent.getInsets(); 
    ...
    vsbR.x = availR.x + availR.width - vsbR.width; vsbR.y = availR.y; 
    if (vsb != null) { 
        vsb.setBounds(vsbR); 
    }
    // horizontal scroll bar 
    Rectangle hsbR = new Rectangle();
    hsbR.height = SB_SIZE; hsbR.width = availR.width - (vsbNeeded ? hsbR.height : 0); 
    hsbR.x = availR.x; hsbR.y = availR.y + availR.height - hsbR.height; 
    if (hsb != null) { hsb.setBounds(hsbR); } } }); 
    // Layering 
    setComponentZOrder(getVerticalScrollBar(), 0); 
    setComponentZOrder(getHorizontalScrollBar(), 1); 
    setComponentZOrder(getViewport(), 2); viewport.setView(view); 
    } 
    ...
    private static class ModernScrollBarUI extends BasicScrollBarUI { 
        ...
        @Override protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
            ...
            graphics2D.fillRect(x, y, width, height); graphics2D.dispose(); 
        } 
        @Override 
        protected void setThumbBounds(int x, int y, int width, int height) { 
            super.setThumbBounds(x, y, width, height); sp.repaint(); 
        } 
    ...
    public static void main(String[] args) { 
       ...
       second.setPreferredSize(new Dimension(800, 800)); 
       second.add(new JLabel("Prueba"));
//       content.add(second); 
       content.add(new ModernScrollPane(second)); 
      frame.add(content);
